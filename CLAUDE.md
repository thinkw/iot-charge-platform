...
| 阶段5：运营后台与告警开发 | ⏳ 待开发 | 站点管理、设备管理、告警管理、数据统计 |
| 阶段6：实时推送与大屏开发 | ⏳ 待开发 | WebSocket推送、实时数据大屏 |
...
- [ ] `AdminStationController` — 充电站增删改查（`/api/admin/station`）
- [ ] `AdminChargerController` — 充电桩增删改查（`/api/admin/charger`）
- [ ] `AdminAlarmController` — 告警列表查询/处理（`/api/admin/alarm`）
- [ ] `AdminStatisticsController` — 数据统计接口（`/api/admin/statistics`）
- [ ] `AlarmService` — 告警业务服务（告警列表查询、处理、统计）
- [ ] `StatisticsService` — 数据统计服务（订单量、营收、充电量趋势）
