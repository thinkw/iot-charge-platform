      ...
      <el-form :inline="true" :model="query" size="default">
        ...
        <el-form-item label="订单状态">
          <el-select v-model="query.orderStatus" placeholder="全部" clearable style="width:120px">
            ...
            <el-option label="待支付" :value="5" /><el-option label="等待设备" :value="6" />
          </el-select>
        </el-form-item>
      ...
      <el-table :data="list" v-loading="loading" border stripe>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.orderStatus === 1 || row.orderStatus === 4 || row.orderStatus === 5 || row.orderStatus === 6" size="small" type="warning" @click="openForceEnd(row)">
              强制结束
            </el-button>
...
function orderStatusText(s: number) { const m: Record<number, string> = { 1: '充电中', 2: '已完成', 3: '已取消', 4: '异常', 5: '待支付', 6: '等待设备' }; return m[s] || '未知' }
